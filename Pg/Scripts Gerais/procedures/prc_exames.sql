CREATE OR REPLACE FUNCTION PRC_EXAMES(V_POSTO                      IN EXAMES.POSTO%TYPE,
                                       ...
                                       V_DATA_AUTORIZACAO           IN EXAMES.DATA_ATORIZACAO%TYPE,
                                       V_ID_TIPO_COLETOR            IN EXAMES.ID_TIPO_COLETOR%TYPE,
                                       V_VALOR_HONORARIOS           IN EXAMES.VALOR_HONORARIOS%TYPE,
                                       V_VALOR_CUSTO_OPERACIONAL    IN EXAMES.VALOR_CUSTO_OPERACIONAL%TYPE,
                                       V_VALOR_PORTE_MEDICO         IN EXAMES.VALOR_PORTE_MEDICO%TYPE,
                                       V_DESCONTO_HONORARIOS        IN EXAMES.DESCONTO_HONORARIOS%TYPE,
                                       V_DESCONTO_CUSTO_OPERACIONAL IN EXAMES.DESCONTO_CUSTO_OPERACIONAL%TYPE,
                                       V_ID_MEDICO_REALIZANTE1      IN EXAMES.ID_MEDICO_REALIZANTE1%TYPE,
                                       V_ID_MEDICO_REALIZANTE2      IN EXAMES.ID_MEDICO_REALIZANTE2%TYPE,
                                       V_FUNCAO                     IN INT) RETURNS VOID AS $$
BEGIN
	IF V_FUNCAO = 1 THEN                                      
		IF V_CORREL < 1 THEN
		     SELECT COALESCE(MAX(CORREL)+1,1) INTO V_CORREL FROM EXAMES
			  WHERE POSTO = V_POSTO 
		...
		INSERT INTO EXAMES
			VALUES (V_POSTO,     V_ATENDIMENTO,  V_CORREL,         V_CONTROLE,  V_MED_REA1,
					...
					V_DATA_AUTORIZACAO, V_ID_TIPO_COLETOR, V_VALOR_HONORARIOS, V_VALOR_CUSTO_OPERACIONAL,
                    V_VALOR_PORTE_MEDICO, V_DESCONTO_HONORARIOS, V_DESCONTO_CUSTO_OPERACIONAL, 
					V_ID_MEDICO_REALIZANTE1, V_ID_MEDICO_REALIZANTE2);
	...
	ELSIF V_FUNCAO = 2 THEN
        UPDATE EXAMES SET CONTROLE                    = V_CONTROLE,
						  ...
						  ID_TIPO_COLETOR             = V_ID_TIPO_COLETOR,
						  VALOR_HONORARIOS            = V_VALOR_HONORARIOS,
						  VALOR_CUSTO_OPERACIONAL     = V_VALOR_CUSTO_OPERACIONAL,
						  VALOR_PORTE_MEDICO          = V_VALOR_PORTE_MEDICO,
						  DESCONTO_HONORARIOS         = V_DESCONTO_HONORARIOS,
						  DESCONTO_CUSTO_OPERACIONAL  = V_DESCONTO_CUSTO_OPERACIONAL,
						  ID_MEDICO_REALIZANTE1       = V_ID_MEDICO_REALIZANTE1,
						  ID_MEDICO_REALIZANTE2       = V_ID_MEDICO_REALIZANTE2
	...
	ELSIF V_FUNCAO = 5 THEN
        UPDATE EXAMES SET GUIA = V_GUIA, SENHA = V_SENHA, AUTORIZACAO = V_AUTORIZACAO, DATA_AUTORIZACAO = V_DATA_AUTORIZACAO
         WHERE POSTO = V_POSTO AND ATENDIMENTO = V_ATENDIMENTO AND CORREL = V_CORREL;
	...
	ELSIF V_FUNCAO = 14 THEN
        UPDATE EXAMES SET AUTORIZACAO = V_AUTORIZACAO,
		                  DATA_ATURIZACAO = V_DATA_AUTORIZACAO
         WHERE POSTO = V_POSTO AND ATENDIMENTO = V_ATENDIMENTO AND CORREL = V_CORREL;
